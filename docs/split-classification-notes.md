...
3. **Parcel area** (HCAD `Shape.STArea()` / HAR `lot`): ≤ 4,000 → Split,
   ≥ 5,000 → Single, conflict between the two sources → review.
4. **Chain marker** demoted to corroboration only — it distinguishes a
   two-permit project, not a subdivided parcel.
5. Otherwise → review. Never default to Split.
...
signal agreeing on Single. **163 of the 398 DATA records currently carry
neither `ty` nor `prod`**, and the weekly auto-ingest
...
- 27 records where HAR `lot` and HCAD `Shape.STArea()` disagree
  (HAR ≤ 4,000, HCAD ≥ 5,000). Currently labelled Single; the rule
  declines to rule. Not investigated.
