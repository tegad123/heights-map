...
def run(args):
    ...
    with sync_playwright() as pw:
        ...
        for name in PAGES:
            ...
            result=p.evaluate('''()=>{
                renderLegend();
                const rows=DATA.map(r=>({id:r.id,address:r.a,product:prodKeyR(r)||'Unknown',weight:UW(r.id),phase:homePhase(r.id),category:inventoryCategory(r.id),twins:r._twin?[r._twin.id]:[],client:r.product_classification?.source==='client',twinClient:r._twin?.product_classification?.source==='client',twinProduct:r._twin?.prod,classified:!!r.product_classification,coords:[r.lat,r.lng],marker:mById[r.id]?.getLatLng(),permits:(r.permits||[]).map(p=>p.proj),tags:pt(r.id).tags}));
                ...
                return {rows,products:rows.reduce((n,r)=>{n[r.product]=(n[r.product]||0)+r.weight;return n;},{}),
                    ...
                    soldRows:SOLD_DATA,posEdits:state.posEdits,fieldEdits:state.fieldEdits};}''')
            ...
            if not args.baseline:
                clients=[['pmt_1438-dian-st-a-77008','pmt_1438-dian-st-b-77008'],['pmt_845-w-23rd-st-a-77008','pmt_845-w-23rd-st-b-77008'],['pmt_1807-palmetto-landing-dr-77009','pmt_1809-palmetto-landing-dr-77009']]
                for pair in clients:
            ...
            if args.live:
                p.evaluate("()=>{map.fitBounds(L.latLngBounds(ZONE_RING.map(([lng,lat])=>[lat,lng])));L.polygon(ZONE_RING.map(([lng,lat])=>[lat,lng]),{color:'#000',weight:3,fill:false}).addTo(map);}")
                p.wait_for_timeout(400);p.screenshot(path=str(Path(args.output).with_name('live-zone-extent.png')))
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--output',required=True);ap.add_argument('--shared');ap.add_argument('--ref');ap.add_argument('--live');ap.add_argument('--baseline',action='store_true');run(ap.parse_args())
